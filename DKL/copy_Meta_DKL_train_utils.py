...
class DynamicNN(nn.Module):
    ...
    '''
    def __init__(self, architecture, input_dim, joint_gp_training_phase=False):
...
class GPRegressionModel_DKL(gpytorch.models.ExactGP):
        ...
        def __init__(self, train_x, train_y, likelihood, feature_extractor, append_sim_column=True):
            super(GPRegressionModel_DKL, self).__init__(train_x, train_y, likelihood)
            self.mean_module = gpytorch.means.ConstantMean()
            self.covar_module = gpytorch.kernels.GridInterpolationKernel( # this is to approx kernel
                #gpytorch.kernels.ScaleKernel(gpytorch.kernels.spectral_mixture_kernel(num_mixtures=4)),
        ...
        def forward(self, x):
            ...
            mean_x = self.mean_module(projected_x)
            covar_x = self.covar_module(projected_x)
            #covar_x = covar_x.add_jitter(self.jitter_value )  # Adds jitter directly
            
            return gpytorch.distributions.MultivariateNormal(mean_x, covar_x)
        

        
def optimize_acq_DKL(func, m, m1,l,l1, fixed, num_f, X):
    #random.seed(seed)
    #np.random.seed(seed)
    #torch.manual_seed(seed)
    ...
    bounds = [(0, 1) for _ in range(X.shape[1] - num_f)]
    for ii in range(T):
        x0 = np.random.uniform(0, 1, X.shape[1] - num_f)
    ...
    # preds = predict(m, l, xtest)
    # print(preds.mean, preds.variance)
    # preds = predict(m1, l1, xtest)
    # print(preds.mean, preds.variance, ' Those are M1s')
...
def UCB_DKL(m, m1,l,l1, x, fixed, kappa=None):
    ...
    try:
        ...
        var = 0
        print('error in varaince')
    #print(mean + kappa * var, 'value of acqui', mean, var, 'for test pt: ' ,xtest)
...
def train_gp_variance(model, likelihood, train_x, train_y):
    model.train()
    likelihood.train()
...
def create_optimizer(model, lr, freeze, warm_start_only):
    ...
    return torch.optim.Adam([ 
    {'params': model.feature_extractor.parameters(), 'lr': lr * 0.1},  # Smaller learning rate for feature extractor
    {'params': model.covar_module.parameters(), 'lr': lr},             # Default learning rate for GP components
...
def evaluate_model(model, likelihood, X_val, y_val):
    y_pred = predict(model, likelihood, X_val)
    pred_mean = y_pred.mean
    pred_var = y_pred.variance
    rmse = torch.sqrt(torch.mean((pred_mean - y_val) ** 2)).item() 
    nll = -likelihood.log_marginal(y_val, y_pred).mean().item()  # Take the mean NLL
        
    return rmse, nll

def metatrain_DKL(model, X_train, y_train, likelihood, 
                         training_iterations=100, freeze=False, 
                         warm_start_only=False, X_val=None, y_val=None,  
                         lr=0.01, patience=30): 
    ...
    optimizer = create_optimizer(model, lr, freeze, warm_start_only)
    ...
    for i in iterator:
        ...
        if X_val is not None and y_val is not None:
            ...
            rmse, nll = evaluate_model(model, likelihood, X_val, y_val)
            
            # Check for improvement in validation loss
            if nll < best_val_loss:
                best_val_loss = nll
                patience_counter = 0  # Reset patience counter
                # Save the best model's state
                best_model_state = model.state_dict()  # Save current model state
            else:
                patience_counter += 1  # Increment patience counter

            # Early stopping check
            if patience_counter >= patience:
                print(f"Early stopping triggered at iteration {i}. Best validation NLL: {best_val_loss}")
                break

            print({'train_loss': loss.item(), 'val_set_rmse': rmse, 
                   'training_itr': i, 'negative_log_likelihood': nll})
    ...
    # Restore the best model state if one exists
    if best_model_state is not None:
        model.load_state_dict(best_model_state)
        print("Restored model to best validation NLL.")
...
def metatrain_DKL_wilson(model, X_train, y_train, likelihood, seed, 
                         training_iterations=50, freeze=False, 
