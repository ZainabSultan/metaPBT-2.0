...
import gpytorch
import random
from DKL.meta_data import MetadataCollection
...
def _select_config(
    ...
    bounds: dict,
    num_f: int,
    neural_network: LargeFeatureExtractor,
    seed,
    context=None
) -> np.ndarray:
    ...
    # TODO fix this so that we can pretrain before training end to end??
    # X_original = normalize(Xraw, limits)
    # y_original = standardize(yraw).reshape(yraw.size, 1)

    # cross_val, spearman_corr = train_neural_network_model(model=neural_network, x=X_original, y=y_original )
    # print(cross_val, spearman_corr)
    # feature_extractor = neural_network.feature_extractor
    ...
    # X_dkl = feature_extractor(X)

    # kernel = TV_SquaredExp(
    #     input_dim=X_dkl.shape[1], variance=1.0, lengthscale=1.0, epsilon=0.1
    # )
    ...
    m_trained, mll_m ,l= train_DKL_wilson(model=m, X_train=train_x, y_train=train_y, likelihood=likelihood,seed=seed)
    ...
    if current is None:
        ...
        # lengthscale=1.) this was always commented btw
        ...
        m1_trained, mll_m1,l1 = train_DKL_wilson(model=m1, X_train=train_xnew, y_train=train_ynew, likelihood=likelihood, freeze=True,seed=seed)
...
def _explore(
    ...
    if not dfnewpoint.empty:
        ...
        Xmeta, ymeta, context = meta_data
        print(len(Xmeta))
        print('BEFORE', X.shape, np.array(Xmeta).shape, np.array(Xmeta))
        np.append(X, Xmeta, axis=0)
        np.append(y, np.array(ymeta).flatten(), axis=0)
        print('X after', X.shape, Xmeta.shape, y.shape )
        new = _select_config(X, y, current, newpoint, bounds, num_f=len(t_r.columns), neural_network= neural_network, seed=seed, context=context)
...
class PB2_Meta_dkl(PopulationBasedTraining):
    ...
    def __init__(
        ...
        neural_network: LargeFeatureExtractor = LargeFeatureExtractor,
        seed:int =None,
        meta_data_collection: MetadataCollection = None,
        ...
        super(PB2_Meta_dkl, self).__init__(
            time_attr=time_attr,
            metric=metric,
            mode=mode,
        ...
        num_inputs = 2 + len(hyperparam_bounds) # time, reward and num of HPs
        self.neural_network = LargeFeatureExtractor(data_dim=num_inputs, seed=seed) 
        self.meta_data_collection = meta_data_collection
    ...
    def _save_trial_state(
        ...
        score = super(PB2_Meta_dkl, self)._save_trial_state(state, time, result, trial)
    ...
    def _get_new_config(self, trial: Trial, trial_to_clone: Trial) -> Tuple[Dict, Dict]:
        ...
        meta_data = self.meta_data_collection.find_before_time_step(self.data["Time"].max())
        #print('meta data', meta_data)
        new_config_flat, data = _explore( #is the addition here???
            self.data,
