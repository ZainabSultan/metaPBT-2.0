...
def get_metadata_dirs(env_config, num_meta_contexts, method='closest', exps_dir='', num_envs=22):
    if exps_dir=='':
    ...
    id_list = np.arange(1,num_envs)
    
    sorted_ids = sorted(id_list, key=lambda id_val: abs(id_val - id))
    if method == 'closest':
        ids = sorted_ids[:num_meta_contexts]
    elif method == 'gen':
        ids = np.linspace(1, num_envs, num_meta_contexts, dtype=int)
        print(ids)

    target_envs = [f'{env_name}.{feature}.c{i}' for i in ids]
    target_envs.extend([f'{env_name}_{feature}_context_{i}' for i in ids]) # alternative name
    print('I am env', id, 'and my sources of info are: ', ids)
