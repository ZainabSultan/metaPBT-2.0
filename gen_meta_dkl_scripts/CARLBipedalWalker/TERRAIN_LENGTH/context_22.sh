...
trap on_exit EXIT
# 
unzip /pfs/work7/workspace/scratch/fr_zs53-dkl_exps/CARLBipedalWalker_4_agents_TERRAIN_LENGTH.zip -d $TMPDIR/
cd /home/fr/fr_fr/fr_zs53/DKL/metaPBT-2.0/

# Loop over seeds and run jobs with the same context
context='{"TERRAIN_LENGTH":440.00000000000006}'
for seed in {0..9}; do
    kwargs="--ws_dir=$tmp_logs_dir --seed=$seed --env_name=CARLBipedalWalker --num_samples=4 --context='$context' --max=1000000 --t_ready=50000 --num_meta_envs=3 --meta_selection_method=gen --meta_data_base_dir=$TMPDIR/CARLBipedalWalker_4_agents/TERRAIN_LENGTH/"
