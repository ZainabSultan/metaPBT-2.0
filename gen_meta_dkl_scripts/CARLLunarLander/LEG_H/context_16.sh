...
trap on_exit EXIT
# 
unzip /pfs/work7/workspace/scratch/fr_zs53-dkl_exps/CARLLunarLander_4_agents_LEG_H.zip -d $TMPDIR/
cd /home/fr/fr_fr/fr_zs53/DKL/metaPBT-2.0/

# Loop over seeds and run jobs with the same context
context='{"LEG_H":12.8}'
for seed in {0..9}; do
    kwargs="--ws_dir=$tmp_logs_dir --seed=$seed --env_name=CARLLunarLander --num_samples=4 --context='$context' --max=1000000 --t_ready=50000 --num_meta_envs=5 --meta_selection_method=gen --meta_data_base_dir=$TMPDIR/CARLLunarLander_4_agents/LEG_H/"
