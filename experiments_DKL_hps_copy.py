...
from DKL.Meta_DKL_train_utils import UCB_DKL, metatrain_DKL,  GPRegressionModel_DKL, optimize_acq_DKL, predict
...
def dynamic_exp(config):
    ...
    if True:
        ...
        for i in range(t_perturb_simulation, len(x_test), t_perturb_simulation):
            ...
            rsme_meta_model, model_trained, l = create_and_test_meta_model(Xraw, yraw, config, neural_network, X, y, x_test_exp, y_test_exp)
            rsme_baseline, xt = create_and_test_baseline_model(Xraw, yraw, config, x_test_exp, y_test_exp)

            print('this is what the model thinks, ', predict(model_trained, l, xt).mean)
...
def create_and_test_meta_model(Xraw, yraw, config, neural_network, meta_x, meta_y, x_test, y_test):
    ...
    m = GPRegressionModel_DKL(train_x, train_y, feature_extractor=neural_network, likelihood=likelihood)
    m_trained, mll_m ,l= metatrain_DKL(model=m, X_train=train_x, y_train=train_y, likelihood=likelihood, X_val=X_val, y_val=y_val)
    # train a second model for the variance
    lv = gpytorch.likelihoods.GaussianLikelihood()
    m_variance = ExactGPModel(train_x, train_y, lv)
    m_variance_trained, lv = train_gp_variance(m_variance, lv, train_x, train_y)
    ...
    m_variance_trained.eval()
    lv.eval()
    with torch.no_grad(), gpytorch.settings.use_toeplitz(False), gpytorch.settings.fast_pred_var():
        preds_v = m_variance_trained(x_test)
    print(pd.DataFrame(data = preds_v.variance, columns = ['metapreds2.0']).describe())
    print(pd.DataFrame(data = preds.mean, columns = ['metapreds_mean']).describe())
    print(f'RSME META MODEL: {rmse}') 
    print('correlation', np.corrcoef(preds.mean, preds.variance), preds.mean.shape)
    print('same gp for mean and var ')
    optimize_acq_DKL(UCB_DKL, m_trained, m_trained,l,l, normalize(Xraw[-1,:3], oldpoints), 3 , X)
    print('variance should be better here, so better preds ? ')
    optimize_acq_DKL(UCB_DKL, m_trained, m_variance_trained,l,lv,normalize(Xraw[-1,:3], oldpoints), 3 , X)
    
    return rmse, m_trained, l
...
def create_and_test_baseline_model(Xraw, yraw, config, x_test, y_test):
    ...
    #print(corr)
    xt=optimize_acq(UCB, m, m,  normalize(Xraw[-1,:2], oldpoints), 2)
    xtest = np.concatenate((normalize(Xraw[-1,:2], oldpoints).reshape(-1, 1), np.array(xt).reshape(-1, 1))).T 
    xtest = xtest.astype(np.float32)
    print(xtest)

    xtest = np.insert(xtest, 2, 0.0, axis=1)  # Insert 99 at index 3 along axis 1
