...
trap on_exit EXIT
# 
unzip /pfs/work7/workspace/scratch/fr_zs53-dkl_exps/CARLCartPole_4_agents_length.zip -d $TMPDIR/
cd /home/fr/fr_fr/fr_zs53/DKL/metaPBT-2.0/

# Loop over seeds and run jobs with the same context
context='{"length":0.45}'
for seed in {0..5}; do
    kwargs="--ws_dir=$tmp_logs_dir --seed=$seed --env_name=CARLCartPole --num_samples=4 --context='$context' --max=100000 --t_ready=5000 --num_meta_envs=3 --meta_selection_method=closest --meta_data_base_dir=$TMPDIR/CARLCartPole_4_agents/length/"
